...
def get_sightengine_credentials():
    ...
    if _sightengine_credentials is None:
        ...
        missing = [
            name
            for name in [SSM_SIGHTENGINE_API_USER, SSM_SIGHTENGINE_API_SECRET]
            if not _sightengine_credentials.get(name)
        ]
        if missing:
            raise RuntimeError(f'Missing Sightengine SSM parameters: {missing}')
...
def get_article_analysis(conn, article_id: int, image_hash: str) -> dict:
    """Return an existing result for the same article and image."""
    with conn.cursor() as cursor:
        cursor.execute(
            '''
            SELECT id, ai_probability, api_name
            FROM image_analyses
            WHERE article_id = %s AND image_hash = %s
            LIMIT 1
            ''',
            (article_id, image_hash),
        )
        return cursor.fetchone()
...
def process_message(message: dict) -> dict:
    ...
    try:
        for image_url in image_urls:
            ...
            # SQS retries must not duplicate the same article/image result.
            article_existing = get_article_analysis(conn, article_id, image_hash)
            if article_existing:
                result['reused'] += 1
                print(f'  Already processed: {image_url[:50]}...')
                continue
